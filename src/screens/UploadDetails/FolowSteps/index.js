...
const FolowSteps = ({ className, uploadState, hashValue, itemName, itemDesc, itemSize, itemCollection, setVisibleModal }) => {
  ...
  const mintNFT = async () => {
    ...
    const networkId = await web3.eth.net.getId();console.log("networkId", networkId);
    ...
    console.log("PhotoNFTFactory", PhotoNFTFactory);
    console.log("accounts", accounts[0], fee);
    ...
    PhotoNFTFactory.methods.createNewPhotoNFT(itemName, price, hashValue, itemDesc, itemCollection, itemSize)
      ...
      .once("receipt", async (receipt) => {
        console.log("receipt origin ++++++", receipt);
        console.log("receipt++++++", receipt.events.PhotoNFTCreated.length);
