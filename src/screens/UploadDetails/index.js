...
const Upload = (props) => {
  ...
  useEffect(async () => {
    const collections = await getCollections(); console.log("collections", collections)
    ...
    if (window.ethereum != null && connectFlag != null) {
  ...
  async function imageChange(e) {
    if (e.target.files && e.target.files.length > 0) {
      ...
      //== add ==
      const file = e.target.files[0]; console.log("ipfs link", file)
      try {
